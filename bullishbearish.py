...
############################################################################
# 1) HARDCODE YOUR HISTORICAL DATASET
#    We'll place all your multi-year data as a Python list-of-dicts.
#    Fill in the entire list from your earlier dataset.
############################################################################
...
DATA_ROWS = [
    ...
    #{'Date':'2025-01-07','Open':197.205,'High':198.207,'Low':196.870,'Close':197.117,'Mid':0.80243},
    #{'Date':'2025-01-08','Open':197.091,'High':197.544,'Low':195.115,'Close':195.648,'Mid':0.80243},
]

############################################################################
# 2) ADD ADVANCED STRATEGY-BASED FEATURES (LiquiditySweep, FairValueGap, etc.)
#    We'll define a placeholder function that modifies the DataFrame in place,
#    exactly like we did in earlier code. You can elaborate the logic as you wish.
############################################################################

def add_advanced_features(df):
    """
    Add advanced strategy-based columns:
    - 'LiquiditySweep'
    - 'FairValueGap'
    - 'OrderBlock'
    - 'ATR'
    ...
    This is a placeholder. We'll do minimal logic for demonstration.
    """
    # 1) Example: ATR ~ average range over last 14
    df['Range'] = df['High'] - df['Low']
    df['ATR']   = df['Range'].rolling(14, min_periods=1).mean()

    # 2) Liquidity sweep => if today's Low < yest's Low but closes above yest's close
    df['prevLow']   = df['Low'].shift(1)
    df['prevClose'] = df['Close'].shift(1)
    cond_ls = (df['Low'] < df['prevLow']) & (df['Close'] > df['prevClose'])
    df['LiquiditySweep'] = cond_ls.astype(int)

    # 3) FairValueGap => if today's Low > yest's High
    df['prevHigh'] = df['High'].shift(1)
    cond_fvg = (df['Low'] > df['prevHigh'])
    df['FairValueGap']   = cond_fvg.astype(int)

    # 4) OrderBlock => if range > 2x 5-day avg range
    df['AvgRange_5'] = df['Range'].rolling(5, min_periods=1).mean()
    cond_ob = df['Range'] > 2.0 * df['AvgRange_5']
    df['OrderBlock'] = cond_ob.astype(int)

    # We can drop the helper columns
    df.drop(columns=['Range','prevLow','prevClose','prevHigh','AvgRange_5'], inplace=True)

    # That's it. We'll keep LiquiditySweep, FairValueGap, OrderBlock, ATR in df.
    return df

############################################################################
# 3) BUILD SUPERVISED DATA: We want to classify "tomorrow is bullish (1) or not (0)."
#    So for day t, X(t) => [Open_t, High_t, Low_t, Close_t, ATR_t, LiquiditySweep_t, etc.]
#    y(t) => 1 if day t+1's close > day t+1's open, else 0
############################################################################

def build_supervised(df):
    """
    We'll shift the label by -1 day:
      y(t) = 1 if Close_{t+1} > Open_{t+1}, else 0
    We'll skip the last row because it has no tomorrow label.
    """
    df['Open_tom']  = df['Open'].shift(-1)
    df['Close_tom'] = df['Close'].shift(-1)

    # The classification label: 1 if tomorrow is bullish
    df['TomorrowBullish'] = (df['Close_tom'] > df['Open_tom']).astype(int)

    # We'll now define which columns are used as features
    # e.g. [Open,High,Low,Close,ATR,LiquiditySweep,FairValueGap,OrderBlock]
    feature_cols = [
        'Open','High','Low','Close','ATR','LiquiditySweep','FairValueGap','OrderBlock'
    ]
    # We'll skip the last row
    df_features = df[feature_cols].iloc[:-1]
    df_target   = df['TomorrowBullish'].iloc[:-1]

    X = df_features.values
    y = df_target.values

    # Also store the final row's feature-values for "today" => used for the prediction (minus label)
    last_features = df[feature_cols].iloc[-1].values

    # Drop the helper columns
    df.drop(columns=['Open_tom','Close_tom','TomorrowBullish'], inplace=True, errors='ignore')
    return X, y, last_features


############################################################################
# 4) TRAIN A CLASSIFIER (e.g. logistic regression) => predict next day bullish/bearish
############################################################################

def train_classifier(X, y):
    """
    We'll do a single pass logistic regression.
    """
...
def main():
    ...
    # 3) BUILD SUPERVISED => get X,y + final row's features
    X, y, last_feat = build_supervised(df)

    # 4) TRAIN CLASSIFIER
    clf = train_classifier(X, y)

    # (Optional) Evaluate in-sample classification accuracy
    y_pred_insample = clf.predict(X)
    insample_acc = np.mean(y_pred_insample == y)
    print(f"=== In-Sample Accuracy: {insample_acc*100:.2f}% ===")

    # 5) ASK FOR THE NEXT DAY's OPEN (the day after the final row in data)
    next_open_str = input("Enter the OPEN price for NEXT day after the last row in data: ")
    try:
        next_open_val = float(next_open_str)
    except:
        print("Invalid. Using last known open as fallback.")
        next_open_val = last_feat[0]  # a fallback

    # We'll use 'last_feat' as the final day's advanced features, 
    # but we override the 'Open' portion with next_open_val (the user-supplied).
    # Our features are in order => [Open, High, Low, Close, ATR, LiqSwep, FVG, OB]
    feat_vec = last_feat.copy()
    feat_vec[0] = next_open_val  # override "Open"

    # 6) GET PREDICTION
    feat_vec_2d = feat_vec.reshape(1,-1)
    pred_prob = clf.predict_proba(feat_vec_2d)[0]  # e.g. [prob(bearish=0), prob(bullish=1)]
    prob_bullish = pred_prob[1]

    # classification
    label = "BULLISH" if (prob_bullish >= 0.5) else "BEARISH"
    confidence = prob_bullish if label=="BULLISH" else (1.0 - prob_bullish)

    print("\n=== FINAL PREDICTION ===")
    print(f"Prediction: {label}")
    print(f"Open Price: {next_open_val:.5f} (the input you provided)")
