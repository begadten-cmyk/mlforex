...
and Continuous Scanning via `schedule`.
...
def ict_full_pipeline(symbol="GBPJPY=X", 
                      ...
                      bias_lookback=15,          # changed from 30 to 15
                      displacement_factor=1.0,   # changed from 1.5 to 1.0
                      buffer_pct=0.003,
                      account_size=10000,
                      risk_per_trade=0.01):
    """
    ...
    2) Detect daily bias => "Bullish", "Bearish", or None (less strict now)
    ...
    if daily_bias == "Bullish" and (sh_price > sl_price):
        ...
        if in_ote:
            ...
            results["reason"] = "Bullish OTE with daily bias (less strict)."
        ...
        if in_ote:
            ...
            results["realized_pnl"] = realized_pnl
            results["tp_message"] = tp_msg
            results["reason"] = "Bearish OTE with daily bias (less strict)."
        ...
        # If daily_bias is None or the range is invalid
        results["signal"] = None
...
def run_ict_pipeline_continuous():
    ...
    for symbol in london_pairs:
        results = ict_full_pipeline(
            ...
            intraday_period="1mo",
            intraday_interval="1h",
            kill_zone_hours=(2, 5),     # London window
            bias_lookback=15,          # or 30, your choice
            displacement_factor=1.0,   # or 1.5, etc.
            buffer_pct=0.003,
            account_size=10000,
            risk_per_trade=0.01
        )
    ...
    for symbol in ny_pairs:
        results = ict_full_pipeline(
            ...
            intraday_period="1mo",
            intraday_interval="1h",
            kill_zone_hours=(7, 10),    # New York window
            bias_lookback=15,
