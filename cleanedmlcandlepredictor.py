...
def train_and_predict_for_pair(pair):
    print(f"\n=== PROCESSING {pair} ===")
    df = yf.download(pair, start=start_date, end=end_date, interval="1d")
    if df.empty:
    ...
    # SHIFT: create tomorrow's high/low
    df['Tomorrow_High'] = df['High'].shift(-1)
    df['Tomorrow_Low']  = df['Low'].shift(-1)
    df.dropna(inplace=True)

    # Basic feature engineering
    # We'll do a log transform of the 'Close' to handle scale
    ...
    # LOG transform the targets (tomorrow's High, Low)
    # We must ensure they're > 0
    df = df[(df['Tomorrow_High']>0) & (df['Tomorrow_Low']>0)]
    ...
    # We'll fill MA_5, MA_10, or volume NAs with e.g. 0 if needed
    df[feature_cols] = df[feature_cols].fillna(0)

    X = df[feature_cols].values
    Y = df[['LogTomorrow_High','LogTomorrow_Low']].values  # multi-output
    ...
    # We'll exponentiate predictions to get back to real price scale
    ...
    print(f"{pair} - Test RMSE High: {rmse_high:.4f}, Low: {rmse_low:.4f}")

    # Let's do one final "latest row" prediction
    print(df.tail(5))
    last_row = df.iloc[-1]
    X_latest = last_row[feature_cols].values.reshape(1, -1)
    pred_log = model.predict(X_latest)[0]
    pred_high = np.exp(pred_log[0])
    pred_low  = np.exp(pred_log[1])

    print(f"Latest known close: {last_row['Close']:.5f}")
    print(f"Predicted Tomorrow's High: {pred_high:.5f}, Low: {pred_low:.5f}")
