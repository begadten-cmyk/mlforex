...
def detect_daily_bias(df, lookback=15, displacement_factor=1.0):
    """
    A more ICT-aligned daily bias detection function, now more lenient:
      1) Identify major high & low in the last `lookback` bars. (Now 15, was 30)
      ...
      3) We removed the discount/premium requirement to be less strict.
    """
    ...
    displacement = (last_body_size > displacement_factor * avg_body)  # now displacement_factor=1.0
    ...
    # NEW: No discount/premium check, just return "Bullish" or "Bearish" if shift detected.
    if bullish_shift:
        return "Bullish"
    elif bearish_shift:
        return "Bearish"
...
def fetch_and_filter_15m(symbol, kill_zone, intraday_period="1mo"):
    """
    Fetch 15-minute data for the same symbol, convert to NY time,
    filter by the same kill zone. Returns the filtered 15m DataFrame.
    """
...
def ict_full_pipeline(symbol="GBPJPY=X", 
    ...
    1) Fetch Daily + 1H Intraday
    2) Detect daily bias => "Bullish", "Bearish", or None
    3) Mark daily liquidity (swing highs/lows)
    4) Intraday OB/FVG + Kill Zone (1H)
    5) Also fetch & filter 15M data in the SAME kill zone
    6) OTE check (both 1H & 15M must align) for final trade
    7) Stop + position sizing
    8) Partial profits
    9) Telegram notification if a signal is triggered
    """
    ...
    # For final signal, we want OTE alignment on 1H + 15M
    # We'll check OTE on 1H, then confirm 15M last candle also in OTE.

    ########## STEP 6: OTE Check (Bullish or Bearish) on 1H + 15M ##########
    if daily_bias == "Bullish" and (sh_price > sl_price):
        in_ote_1h, ote_zone_1h = is_price_in_bullish_ote(kz_df_1h, sl_price, sh_price, tolerance=0.0001)
        in_ote_15m, ote_zone_15m = is_price_in_bullish_ote(kz_df_15m, sl_price, sh_price, tolerance=0.0001)

        results["in_ote_1h"] = in_ote_1h
        results["in_ote_15m"] = in_ote_15m

        if in_ote_1h and in_ote_15m:
            # We'll use last close on the 15m chart for entry, for more precise entry.
            ...
            results["reason"] = "Bullish OTE on 1H + 15M alignment."
            ...
            results["signal"] = None
            results["reason"] = "Bullish bias but OTE not aligned on both 1H & 15M."
            return results

    elif daily_bias == "Bearish" and (sh_price > sl_price):
        in_ote_1h, ote_zone_1h = is_price_in_bearish_ote(kz_df_1h, sh_price, sl_price, tolerance=0.0001)
        in_ote_15m, ote_zone_15m = is_price_in_bearish_ote(kz_df_15m, sh_price, sl_price, tolerance=0.0001)
        ...
        if in_ote_1h and in_ote_15m:
            ...
            results["realized_pnl"] = realized_pnl
            results["tp_message"] = tp_msg
            results["reason"] = "Bearish OTE on 1H + 15M alignment."
            ...
            results["signal"] = None
            results["reason"] = "Bearish bias but OTE not aligned on both 1H & 15M."
            return results
...
def run_ict_pipeline_continuous():
    """
    Runs the ICT pipeline for multiple pairs, using 2–5 am for EUR/GBP pairs
    and 7–10 am for USD-based pairs.
    """
    # Pairs that should use 2–5 am (London session)
    london_pairs = [
        "GBPJPY=X",
        "EURUSD=X",
        "GBPUSD=X",
    ...
    # Pairs that should use 7–10 am (NY session)
    ny_pairs = [
        "USDCAD=X",
        "USDJPY=X",
        "GBPJPY=X"
    ...
    # 1) Process London pairs with kill zone (2,5)
    for symbol in london_pairs:
        results = ict_full_pipeline(
            ...
            intraday_period="1mo",
            intraday_interval="1h",
            kill_zone_hours=(1, 6),  # London window
            bias_lookback=15,
            displacement_factor=1.0,
            buffer_pct=0.003,
            account_size=10000,
    ...
    # 2) Process NY pairs with kill zone (7,10)
